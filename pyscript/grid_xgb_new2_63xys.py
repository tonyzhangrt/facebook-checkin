...
TRAIN_TEST_HASH_FILE = '../input/train_test_hash_63_159_xys.csv'

timestamp =  datetime.datetime.now().strftime('%Y%m%d%H%M%S')

VALID_LOG_FILE = '../valid/valid_log_xgb_new2_63xys_{0:s}.csv'.format(timestamp)
VALID_OUTPUT_FILE = '../valid/valid_score_xgb_new2_63xys_{0:s}.csv'.format(timestamp)
TEST_OUTPUT_FILE = '../train/test_score_xgb_new2_63xys_{0:s}.csv'.format(timestamp)
SUBMISSION_FILE = '../output/submission_xgb_new2_63xys_{0:s}.csv'.format(timestamp)
...
def run_local_xgb(local_row_id_train, local_Xtrain, local_place_id_train, local_Xtrain_time, local_row_id_test, local_Xtest, cur_hash):
    ...
    valid_train_rows = np.squeeze(np.argwhere(local_Xtrain_time <= valid_split))
    valid_test_rows = np.squeeze(np.argwhere(local_Xtrain_time > valid_split))
...
def run_grid_xgb():
    ...
    while 1:
        ...
        if line[0] == '^':
            ...
            local_Xtrain_time = local_Xtrain[:, 3].copy()
                        
    #         local_Xtrain[:, 4] = np.remainder(local_Xtrain[:, 3], 24 * 60)
    #         local_Xtrain[:, 5] = np.remainder(local_Xtrain[:, 3], 24 * 60 * 7)            
                
            local_Xtrain[:, 3] = np.floor_divide(local_Xtrain_time, 60) # hour
            local_Xtrain[:, 4] = np.floor_divide(local_Xtrain[:, 3], 24) # day
            local_Xtrain[:, 5] = np.floor_divide(local_Xtrain[:, 4] , 7) # month
            local_Xtrain[:, 6] = np.floor_divide(local_Xtrain[:, 4], 364) # year
            local_Xtrain[:, 3] = np.remainder(local_Xtrain[:, 3], 24) + \
                                  np.remainder(local_Xtrain_time, 60)/60.0 # hour
            local_Xtrain[:, 4] = np.remainder(local_Xtrain[:, 4], 7) 
            local_Xtrain[:, 5] = np.remainder(local_Xtrain[:, 5], 52) 
            # local_Xtrain[:, 2] = np.log10(local_Xtrain[:, 2]) # accuracy
            ...
            # loading test data
            local_Xtest = np.zeros((num_test, 7))
            ...
            local_Xtest_time = local_Xtest[:, 3].copy()

            # local_Xtest[:, 4] = np.remainder(local_Xtest[:, 3], 24 * 60)
            # local_Xtest[:, 5] = np.remainder(local_Xtest[:, 3], 24 * 60 * 7)

            local_Xtest[:, 3] = np.floor_divide(local_Xtest_time, 60) # hour
            local_Xtest[:, 4] = np.floor_divide(local_Xtest[:, 3], 24) # day
            local_Xtest[:, 5] = np.floor_divide(local_Xtest[:, 4], 7) # month
            local_Xtest[:, 6] = np.floor_divide(local_Xtest[:, 4], 364) # year
            local_Xtest[:, 3] = np.remainder(local_Xtest[:, 3], 24) + \
                                  np.remainder(local_Xtest_time, 60)/60.0 # hour
            local_Xtest[:, 4] = np.remainder(local_Xtest[:, 4], 7) 
            local_Xtest[:, 5] = np.remainder(local_Xtest[:, 5], 52) 
            # local_Xtest[:, 2] = np.log10(local_Xtest[:, 2]) # accuracy
        
            run_local_xgb(local_row_id_train, local_Xtrain, local_place_id_train, local_Xtrain_time, local_row_id_test, local_Xtest, cur_hash)
